require 'xmlsimple'
require 'pathname'
def uploadOneGps(fileName)
  system 'c:/140225_releaseGPSsplitter/GPS_splitter.exe ' + fileName + ' ' + fileName + '.txt'
  gps = XmlSimple.xml_in(fileName + '.txt' + '.gps', {'KeyAttr' => 'name'})
  #cmd = 'i_view32.exe ' + fileName + '.txt_image_diagnostic_0_0.ppm /convert=' + fileName + '.bmp' 
  cmd = 'i_view32.exe ' + fileName.gsub('/',"\\") + '.txt_image_diagnostic_0_0.ppm /convert=' + fileName.gsub('/', "\\") + '.jpg' 
  p cmd
  p 'c:\\abc'
  system cmd
  cmd = 'copy ' + fileName.gsub('/', '\\') + '.jpg app\\assets\\images\\ /Y'
  system cmd
  return
  ...
  k_measurement['image'] = File.basename(fileName + '.jpg')
